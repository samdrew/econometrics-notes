...
where $y=\frac{Y}{AL}$, $k=\frac{K}{AL}$. If we then assume that capital growth per unit effective labour ($\dot k$) is constant we get
...
```{r, echo=FALSE}
graphsize = c(0,3)
curve(x^0.5, xlab = "k (Capital/EffLabour)", ylab = "y (Output/EffLabour)", xlim = graphsize, ylim= graphsize)
par(new = TRUE)
curve(0.7*x, axes = FALSE, xlab = "", ylab = "", xlim = graphsize, ylim = graphsize)
```

which gives us an equilibrium point $k^*$.
